...
split <- sample.split(train$Affected, SplitRatio = 0.75)
...
balanced.data <- SMOTE(Affected ~., dresstrain, perc.over = 4800, k = 5, perc.under = 1000)

as.data.frame(table(balanced.data$Affected))

library(randomForest)  
library(e1071)

rf <- randomForest(Affected~., ntree = 100, data = balanced.data)
plot(rf)
...
table(dresstest$Affected, predict > 0.5)

library(ROCR)
ROCRpred <- prediction(predict, dresstest$Affected)
